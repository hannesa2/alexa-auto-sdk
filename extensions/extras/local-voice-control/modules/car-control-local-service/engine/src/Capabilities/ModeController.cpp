...
bool ModeController::handleRequest(
    ...
    try {
        ...
        if (name == "AdjustMode") {
            ...
            if (m_ordered) {
                ThrowIfNot(
                    carControlService->adjustModeControllerValue(endpoint->getId(), getInstance(), modeDelta),
                    "Platform interface 'adjustModeControllerValue' failed");
                // Update property value since new value is not retrievable via getModeControllerValue()
                auto oldValue = getPropertyValue();
                for (int oldIndex = 0; oldIndex < m_supportedModes.size(); oldIndex++) {
                    if (m_supportedModes[oldIndex] == oldValue) {
                        int newIndex = oldIndex + modeDelta;
                        if (newIndex < 0) {
                            newIndex = 0;
                        } else if (newIndex >= m_supportedModes.size()) {
                            newIndex = m_supportedModes.size() - 1;
                        }
                        setPropertyValue(m_supportedModes[newIndex]);
                        break;
                    }
                }
            } else {
            ...
            if (std::find(m_supportedModes.begin(), m_supportedModes.end(), mode) != m_supportedModes.end()) {
                ThrowIfNot(
                    carControlService->setModeControllerValue(endpoint->getId(), getInstance(), mode),
                    "Platform interface 'setModeControllerValue' failed");
                // Update property value since new value is not retrievable via getModeControllerValue()
                setPropertyValue(mode);
            } else {
