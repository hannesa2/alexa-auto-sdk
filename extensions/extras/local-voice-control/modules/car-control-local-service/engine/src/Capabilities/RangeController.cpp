...
bool RangeController::handleRequest(
    ...
    try {
        ...
        double rangeValue;
        ThrowIfNot(carControlService->getRangeControllerValue(endpoint->getId(), getInstance(), rangeValue), "Platform interface 'getRangeControllerValue' failed");
        setPropertyValue(rangeValue);
        AACE_DEBUG(LX(TAG).d("endpoint", endpoint->getId()).d("name", name).d("instance", getInstance()).d("value", rangeValue));
        return true;
    }
