...
void EndpointImpl::addObserver(std::weak_ptr<IObserver> observer) {
    ...
    if (auto sharedObs = observer.lock()) {
        ...
        // Notify observer of current gateway
        sharedObs->onEndpointChange(m_endpoints);
    }
}
...
void EndpointImpl::onAVSGatewayChanged(const std::string& avsGateway) {
...
void EndpointImpl::notifyObservers() {
    AACE_INFO(LX(TAG));
    ...
    // Notify observers
    AACE_INFO(LX(TAG).m("Notifying observers"));
