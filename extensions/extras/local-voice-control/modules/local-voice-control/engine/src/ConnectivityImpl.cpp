...
AlexaHybrid::SDK::Result<bool> ConnectivityImpl::isConnectionMetered() const {
    // Default to false for now, but we might want to expose this to platform so OEMs can provide whether connection is metered.
    return AlexaHybrid::SDK::Result<bool>::create( false );
}

AlexaHybrid::SDK::Result<int32_t> ConnectivityImpl::maxDownloadSpeed() const {
    // Default to -1 which means no speed limitation, but we might want to expose this to platform so OEMs can provide the max download speed as they needed.
...
void ConnectivityImpl::notifyObservers() {
    AACE_INFO(LX(TAG));
    // Notify callback
    ...
    // Notify observers
    AACE_INFO(LX(TAG).m("Notifying observers").d("connected", m_connected));
